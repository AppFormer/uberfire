...
public class BatchIndexReuseTest extends BaseIndexTest {

    private static final String REPO_NAME  = "temp-repo-batch-index-reuse-test";
    ...
    public void testSingleBatchIndexExecution() throws IOException, InterruptedException {
        ...
        final FileSystem fs1 = ioService().getFileSystem(URI.create("git://"+REPO_NAME+"/file1"));
        assertNotNull(fs1);

        final FileSystem fs2 = ioService().getFileSystem(URI.create("git://"+REPO_NAME+"/file1"));
        assertNotNull(fs2);

        final FileSystem fs3 = ioService().getFileSystem(URI.create("git://"+REPO_NAME+"/file1"));
        assertNotNull(fs3);
    ...
    private void assertEvents(final int informationMessageCount) {
        ...
        assertContains("Starting indexing of git://"+REPO_NAME+"/ ...",
