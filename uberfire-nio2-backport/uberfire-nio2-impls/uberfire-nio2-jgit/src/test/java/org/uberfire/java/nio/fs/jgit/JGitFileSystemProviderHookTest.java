...
public class JGitFileSystemProviderHookTest extends AbstractTestInfra {
    ...
    public void testInstalledHook() {
        ...
        if ( fs instanceof JGitFileSystem ) {
            ...
            assertThat( hooks[ 0 ].getName() ).isEqualTo( "post-commit" );
            assertThat( hooks[ 0 ].canExecute() ).isTrue();
            assertThat( hooks[ 1 ].getName() ).isEqualTo( "pre-commit" );
            assertThat( hooks[ 1 ].canExecute() ).isTrue();
        }
    }
